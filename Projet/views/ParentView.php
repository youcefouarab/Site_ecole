...
class ParentView extends View {
    ...
    function __construct($id) {
        parent::__construct();
        $this->controller = new Parents();
        $nom = ""; $prenom = ""; $date_n = ""; $email = ""; $adresse = "";
        $tel1 = ""; $tel2 = ""; $tel3 = ""; $id_utilisateur = "";
        if ($id != "new") {
            $parent = $this->controller->getParent($id);
            if ($parent) {
                $nom = str_replace("\"", "'", str_replace("_", " ", $parent["nom"])); 
                $prenom = str_replace("\"", "'", str_replace("_", " ", $parent["prenom"]));
                $date_n = $parent["date_naissance"];
                $email = $parent["email"]; 
                $adresse = str_replace("\"", "'", $parent["adresse"]);
                $tel1 = str_replace("\"", "'", $parent["tel1"]); 
                $tel2 = str_replace("\"", "'", $parent["tel2"]); 
                $tel3 = str_replace("\"", "'", $parent["tel3"]);
                $id_utilisateur = $parent["id_utilisateur"];
            } else {
        ...
        ?>
            ...
            <center>
                <?php
                    if ($id == "new") echo "<br><h1>Nouveau parent</h1><br><br>";
                    else echo "<br><h1>Parent ".$id."</h1><br><br>";
                ?>
                <div class="goodform">
                <?php
                    if ($id == "new") echo '<form action="'.BASE_URL.'parents/ajoutParent" method="POST">';
                    else echo '<form action="'.BASE_URL.'parents/modifParent" method="POST">';
                ?>
